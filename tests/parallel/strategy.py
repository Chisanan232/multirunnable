...
def target_fun(*args, **kwargs) -> str:
    ...
    print(f"pid: {_pid} - {mp.current_process()}")
    print(f"ppid: {_ppid} - {mp.current_process()}")
    print(f"finish_time: {_time} - {mp.current_process()}")
    print(f"pids: {Running_PIDs} - {mp.current_process()}")
    print(f"ppids: {Running_PPIDs} - {mp.current_process()}")
    print(f"finish_times: {Running_Finish_Timestamp} - {mp.current_process()}")
...
class TestProcessPool(PoolRunningTestSpec):
    ...
    def test_async_apply(self, process_pool_strategy: ProcessPoolStrategy):
        # Test for no any parameters
        process_pool_strategy.async_apply(function=target_fun)
